...
public final class ServerModel {
    ...
    public Collection<String> getRegisteredUsers() {
        Collection<String> collection = new TreeSet<>(users.values());
        return collection;
    }
    ...
    public Collection<String> getChannels() {
        Collection<String> channelNames = new TreeSet<>(channels.keySet());
        return channelNames;
    }
    ...
    public Collection<String> getUsersInChannel(String channelName) {
        if (channels.get(channelName) != null) {
            Set<String> channelUsers = channels.get(channelName).getMembers();
            Collection<String> usrs = new TreeSet<>(channelUsers);
            return usrs;
        } else {
            Collection<String> em = new TreeSet<>();
            return em;
        }
    }
    ...
    public Broadcast deregisterUser(int userId) {
        ...
        Set<String> shared = new TreeSet<String>();
        Set<String> chanRemove = new TreeSet<String>();
        for (Map.Entry<String, Channel> entry : channels.entrySet()) {

            Channel c = entry.getValue();
            if (c.getOwner().equals(nickname)) {
                chanRemove.add(entry.getKey());
                shared.addAll(c.getMembers());
            } else if (c.getMembers().contains(nickname)) {
                c.removeMember(nickname);
                shared.addAll(c.getMembers());
            }
        }

        for (String str : chanRemove) {
            channels.remove(str);
        }

        users.remove(userId);
        shared.remove(nickname);

        return Broadcast.disconnected(nickname, shared);
    }
    ...
    public Broadcast changeNickname(NicknameCommand nickCommand) {
        ...
        if (!(isValidName(nickname))) {
            return Broadcast.error(nickCommand,
                    ServerResponse.INVALID_NAME);
        }

        for (String string : users.values()) {
            if (string.equals(nickname)) {
                return Broadcast.error(nickCommand,
                        ServerResponse.NAME_ALREADY_IN_USE);
            }
        }

        for (Map.Entry<String, Channel> ent : channels.entrySet()) {
            Channel c = ent.getValue();
            if (c.getOwner().equals(nickCommand.getSender())) {
                c.setOwnerName(nickname);
            }

            if (c.getMembers().contains(nickCommand.getSender())) {
                c.removeMember(nickCommand.getSender());
                c.addMember(nickCommand.getNewNickname());
            }
        }

        users.put(nickCommand.getSenderId(), nickname);
        return Broadcast.okay(nickCommand, getSharedUsers(nickname));
    }

    private Collection<String> getSharedUsers(String nickname) {
        Collection<String> shared = new TreeSet<String>();
        for (Map.Entry<String, Channel> ent : channels.entrySet()) {
            if (ent.getValue().getMembers().contains(nickname)) {
                shared.addAll(ent.getValue().getMembers());
            }
        }
        return shared;
    }
    ...
    public Broadcast createChannel(CreateCommand createCommand) {
        String chanName = createCommand.getChannel();
        Collection<String> chanOwner = new TreeSet<String>();
        chanOwner.add(createCommand.getSender());
        if (!isValidName(chanName)) {
            return Broadcast.error(createCommand,
                    ServerResponse.INVALID_NAME);
        }
        if (channels.containsKey(chanName)) {
            return Broadcast.error(createCommand,
                    ServerResponse.CHANNEL_ALREADY_EXISTS);
        }
        channels.put(
                chanName, new Channel(createCommand.getSender(),
    ...
    public Broadcast joinChannel(JoinCommand joinCommand) {
        String chanName = joinCommand.getChannel();

        if (!channels.containsKey(chanName)) {
            return Broadcast.error(joinCommand,
                    ServerResponse.NO_SUCH_CHANNEL);
        }
        if (channels.get(chanName).isPriv()) {
            return Broadcast.error(joinCommand,
                    ServerResponse.JOIN_PRIVATE_CHANNEL);
        }
        channels.get(chanName).addMember(joinCommand.getSender());
        Collection<String> chanMembers = new TreeSet<String>(channels.get(chanName).getMembers());
        return Broadcast
                .names(joinCommand, chanMembers,
                        channels.get(chanName).getOwner());
    }
    ...
    public Broadcast sendMessage(MessageCommand messageCommand) {
        String chanName = messageCommand.getChannel();
        Collection<String> chanMembers = new TreeSet<String>();

        if (!channels.containsKey(chanName)) {
            return Broadcast.error(messageCommand,
                    ServerResponse.NO_SUCH_CHANNEL);
        }
        if (!channels.get(chanName).getMembers().contains(messageCommand.getSender())) {
            return Broadcast.error(messageCommand,
                    ServerResponse.USER_NOT_IN_CHANNEL);
        }
        chanMembers.addAll(channels.get(chanName).getMembers());
        return Broadcast.okay(messageCommand, chanMembers);
    }
    ...
    public Broadcast leaveChannel(LeaveCommand leaveCommand) {
        String chanName = leaveCommand.getChannel();
        Collection<String> chanMembers = new TreeSet<String>();
        if (!(channels.keySet().contains(chanName))) {
            return Broadcast.error(leaveCommand,
                    ServerResponse.NO_SUCH_CHANNEL);
        }
        if (!(channels.get(chanName).getMembers().
                contains(leaveCommand.getSender()))) {
            return Broadcast.error(leaveCommand,
                    ServerResponse.USER_NOT_IN_CHANNEL);
        }
        if (leaveCommand.getSender().equals(channels.get(chanName).getOwner())) {
            chanMembers.addAll(channels.get(chanName).getMembers());
            channels.remove(chanName);
            return Broadcast.okay(leaveCommand, chanMembers);
        }
        chanMembers.addAll(channels.get(chanName).getMembers());
        channels.get(chanName).removeMember(leaveCommand.getSender());
        return Broadcast.okay(leaveCommand, chanMembers);
    }
    ...
    public Broadcast inviteUser(InviteCommand inviteCommand) {
        String chanName = inviteCommand.getChannel();

        if (!(channels.get(chanName).isPriv())) {
            return Broadcast.error(inviteCommand,
                    ServerResponse.INVITE_TO_PUBLIC_CHANNEL);
        }
        if (!(channels.containsKey(chanName))) {
            return Broadcast.error(inviteCommand,
                    ServerResponse.NO_SUCH_CHANNEL);
        ...
        if (!(users.containsValue(inviteCommand.getUserToInvite()))) {
            return Broadcast.error(inviteCommand,
                    ServerResponse.NO_SUCH_USER);
        }
        if (!(inviteCommand.getSender().equals(channels.get(chanName).getOwner()))) {
            return Broadcast.error(inviteCommand,
                    ServerResponse.USER_NOT_OWNER);
        }
        channels.get(chanName).addMember(inviteCommand.getUserToInvite());
        Collection<String> chanMembers = new TreeSet<String>(channels.get(chanName).getMembers());
        return Broadcast
                .names(inviteCommand, chanMembers,
                        channels.get(chanName).getOwner());
    }
    ...
    public Broadcast kickUser(KickCommand kickCommand) {
        String chanName = kickCommand.getChannel();

        if (!(channels.containsKey(chanName))) {
            return Broadcast.error(kickCommand,
                    ServerResponse.NO_SUCH_CHANNEL);
        }
        if (!(channels.get(chanName).getMembers().contains(kickCommand.getUserToKick()))) {
            return Broadcast.error(kickCommand,
                    ServerResponse.USER_NOT_IN_CHANNEL);
        }
        ...
        if (!(kickCommand.getSender().equals(channels.get(chanName).getOwner()))) {
            return Broadcast.error(kickCommand,
                    ServerResponse.USER_NOT_OWNER);
        }
        Collection<String> chanMembers = new TreeSet<String>(channels.get(chanName).getMembers());
        channels.get(chanName).removeMember(kickCommand.getUserToKick());
        if (kickCommand.getUserToKick().equals(channels.get(chanName).getOwner())) {
            channels.remove(chanName);
        }
        return Broadcast.okay(kickCommand, chanMembers);
    }
