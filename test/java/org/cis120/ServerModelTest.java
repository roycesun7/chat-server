...
public class ServerModelTest {
    ...
    @Test
    public void testInviteInvalidUser() {
        model.registerUser(0);
        Command create = new CreateCommand(0, "User0", "one", true);
        create.updateServerModel(model);
        Command invite = new InviteCommand(0, "User0", "one", "User1");
    ...
    @Test
    public void testInviteInvalidChannel() {
        model.registerUser(0);
        model.registerUser(1);
        Command create = new CreateCommand(0, "User0", "one", true);
        create.updateServerModel(model);
        Command invite = new InviteCommand(0, "User0", "two", "User1");
        Broadcast expected = Broadcast.error(invite, ServerResponse.NO_SUCH_CHANNEL);
        assertEquals(expected, invite.updateServerModel(model), "broadcast");
    }

    @Test
    public void testChangeOwnerNickname() {
        model.registerUser(0);
        Command create = new CreateCommand(0, "User0", "one", true);
        create.updateServerModel(model);
        Command nickname = new NicknameCommand(0, "User0", "two");
        nickname.updateServerModel(model);
        assertTrue(model.getOwner("one").equals("two"), "should be changed");
        assertTrue(model.getNickname(0).equals("two"));
    }

    @Test
    public void testKicktheOwner() {
        ...
        Command kick = new KickCommand(0, "User0", "one", "User0");
    ...
    public void testKickUserNotInChannel() {
        model.registerUser(0); // add user with id = 0
        model.registerUser(1);
        Command create = new CreateCommand(0, "User0", "one", false);
        create.updateServerModel(model);

        Command kick = new KickCommand(0, "User0", "one", "User1");
    ...
    public void testKickUserNotOwner() {
        ...
        Command kick = new KickCommand(1, "User1", "one", "User0");
